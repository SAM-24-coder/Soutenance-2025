...
# Fonction principale pour la gestion des OUs
function Show-OUManagementGUI {
    ...
    $buttonDeleteOU.Add_Click({ 
        Delete-OU -OUName $comboBoxOUDelete.SelectedItem
    })
    $form.Controls.Add($buttonDeleteOU)

    # Action : Actualiser la liste des OUs
    function RefreshOUList {
        # Exemple de fonction de mise à jour de la liste des OUs
        $comboBoxOUParent.Items.Clear()
        $comboBoxOUDelete.Items.Clear()

        # Récupérer les OUs existantes
        $OUs = Get-ADOrganizationalUnit -Filter *  # Utilisation de la bonne commande
        foreach ($ou in $OUs) {
            $comboBoxOUParent.Items.Add($ou.Name)
            $comboBoxOUDelete.Items.Add($ou.Name)
        }
    }

    # Actualiser la liste des OUs au démarrage
    RefreshOUList
